 ...
 */
#define NEW_CNT_VAL 30
bool invalTest = false;
...
static int be_provider(void)
{
    ...
    AllTypes at(advertiser, NUM_ELEMENTS);
    ...
    cout << "Provider waiting to continue invalidating properties" << endl;
    at.WaitForInvalidateAll();

    cout << "Provider invalidating properties" << endl;
...
static datadriven::Semaphore _semAllTypes; // semaphore for AllTypes properties
static datadriven::Semaphore _semAllArrays; // semaphore for AllArrays properties
static datadriven::Semaphore _semAllDictionaries; // semaphore for AllDictionaries properties
...
class AllTypesListener :
  ...
  public:
     ...
     *       -# validate the invalidated properties
     */
    void OnUpdate(const std::shared_ptr<AllTypesProxy>& atp)
    {
        cout << "Consumer AllTypesProxy received object update for " << atp->GetObjectId() << endl;
        dump("Consumer OnUpdate properties ", atp->GetProperties());
        validate(atp->GetProperties(), _num_elements);
        long long cnt = (invalTest) ? NEW_CNT_VAL : 0;
        validateInvalidate(atp, _num_elements, cnt);
        assert(ER_OK == _semAllTypes.Post());
        cout << "Consumer AllTypesProxy validated object update" << endl;
    }
    ...
    {
        cout << "Consumer AllArraysProxy received object update for " << aap->GetObjectId() << endl;
        dump("Consumer OnUpdate properties ", aap->GetProperties());
        validate(aap->GetProperties(), _num_elements);
        long long cnt = (invalTest) ? NEW_CNT_VAL : 0;
        validateInvalidate(aap, _num_elements, cnt);
        assert(ER_OK == _semAllArrays.Post());
        cout << "Consumer AllArraysProxy validated object update" << endl;
    }
    ...
    {
        cout << "Consumer AllDictionariesProxy received object update for " << adp->GetObjectId() << endl;
        dump("Consumer OnUpdate properties ", adp->GetProperties());
        validate(adp->GetProperties(), _num_elements);
        long long cnt = (invalTest) ? NEW_CNT_VAL : 0;
        validateInvalidate(adp, _num_elements, cnt);
        assert(ER_OK == _semAllDictionaries.Post());
        cout << "Consumer AllDictionariesProxy validated object update" << endl;
    }
...
static void call_method_all_types(const shared_ptr<AllTypesProxy>& proxy)
{
    ...
    std::shared_ptr<datadriven::MethodInvocation<AllTypesProxy::MethodWithAllTypesInAndOutReply> > inv =
        proxy->MethodWithAllTypesInAndOut(true, 2, 3, 4, 5, 6, 7, 8, 9, "10", "iiiiiiiiiii", "/path/12", v, swat);
...
static void call_method_all_arrays(const shared_ptr<AllArraysProxy>& proxy)
{
    ...
    cout << "Consumer validated reply" << endl;
    cout << "Consumer waiting for signal" << endl;
    assert(ER_OK == _semAllArrays.Wait());
    cout << "Consumer done waiting for signal" << endl;
}
...
static void call_method_all_dictionaries(const shared_ptr<AllDictionariesProxy>& proxy)
{
    ...
    cout << "Consumer validated reply" << endl;
    cout << "Consumer waiting for signal" << endl;
    assert(ER_OK == _semAllDictionaries.Wait());
    cout << "Consumer done waiting for signal" << endl;
}
...
/**
 * \test continue test method call. Let the provider know to continue with the test.
 */
static void call_method_invalidate_all(const shared_ptr<AllTypesProxy>& proxy)
{
    cout << "Consumer calling invalidate all method on " << proxy->GetObjectId() << endl;
    invalTest = true;
    std::shared_ptr<datadriven::MethodInvocation<AllTypesProxy::InvalidateAllReply> > inv = proxy->InvalidateAll(
        NEW_CNT_VAL);
    cout << "Consumer waiting for confirmation in reply" << endl;
    const AllTypesProxy::InvalidateAllReply& reply = inv->GetReply();
    assert(datadriven::MethodInvocation<AllTypesProxy::InvalidateAllReply>::READY == inv->GetState());
    assert(ER_OK == reply.GetStatus());
    cout << "Consumer validated invalidate all reply" << endl;
}
...
static int be_consumer(void)
{
    ...
    std::shared_ptr<datadriven::Observer<AllTypesProxy> > observer_at = datadriven::Observer<AllTypesProxy>::Create(
        &atl);
    std::shared_ptr<datadriven::Observer<AllArraysProxy> > observer_aa = datadriven::Observer<AllArraysProxy>::Create(
        &atl);
    std::shared_ptr<datadriven::Observer<AllDictionariesProxy> > observer_ad =
        datadriven::Observer<AllDictionariesProxy>::Create(&atl);
    ...
    assert(3 == cnt);
    cout << "Consumer first part done" << endl;
    ...
    for (datadriven::Observer<AllTypesProxy>::iterator it = observer_at->begin();
        ...
        assert(ER_OK == it->GetStatus());
        cout << "Consumer in iterator for " << it->GetObjectId() << endl;
        call_method_invalidate_all(*it);
    }

    // wait for a notification of the invalidated properties
    cout << "Consumer Wait for invalidated properties" << endl;
    assert(ER_OK == _semAllTypes.Wait()); // wait for inval properties
    assert(ER_OK == _semAllArrays.Wait()); // wait for array inval properties
    assert(ER_OK == _semAllDictionaries.Wait()); // wait for dictionary inval properties
    cout << "Consumer Invalidated properties received" << endl;
