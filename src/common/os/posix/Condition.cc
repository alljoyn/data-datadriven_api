 ...
 * @file
 *
 * This file implements datadriven::Condition for Posix systems
 */
...
QStatus Condition::Wait(datadriven::Mutex& m)
{
...
QStatus Condition::TimedWait(datadriven::Mutex& m, uint32_t ms)
{
...
QStatus Condition::Broadcast()
{
    ...
    if (ret != 0) {
        QCC_LogError(ER_OS_ERROR,
                     ("Condition::Broadcast(): Cannot broadcast signal pthread condition variable (%d)", ret));
        return ER_OS_ERROR;
    }
    return ER_OK;
}
